...
class GjmaaMyAuctionsAllegro {
	...
	public function saveAuctions($auctions,$api,$settings_id,$myLoginId = null){
		...
		foreach($auctions as $auction){
			...
			. (isset($auction->sellerInfo) ? $auction->sellerInfo->userId : $myLoginId) . "),";
			
			if($first)
				$userId = isset($auction->sellerInfo) ? $auction->sellerInfo->userId : $myLoginId;
...
//			209 => 'aukro.ua'
	...
	public function getNameOfSort($sort = null){
		$sorts = array(
			'1_0' => __('Time to end of auction (Ascending)','gj_myauctions_allegro_pro'),
...
//			'my_auctions' => __('My auctions','gj_myauctions_allegro'),
	...
	function showAuctionsFromSettings($attrs,$type = 'widget',$api_allegro){
		extract($attrs);
		$id = $attrs['id'];
		...
		return $this->renderViewAuctions($attrs,$auctions,$settingsById,$type,$api_allegro);
	}
	
	function renderViewAuctions($attrs,$auctions,$settingsById,$type,$api_allegro){
        if($type != 'widget' && isset($attrs['title'])) $html = '<h2>' . $attrs['title'] . '</h2>'; else $html = '';
        $item_allegro_model = new GjmaaAuctionItem();
		$html .= '<div class="module_max_width">';
		if(count($auctions) > 0){
            $user_id = null;
            $first = true;
			foreach($auctions as $auction){
				...
				'<div class="item item-top">'
					.'<p class="allegro_auction" allegro_id="'.$id.'">'
						.'<a class="image_allegro top" target="_blank" href="'.$url.'" title="'.$title.'" style="width: 150px; height: 115px;"><span style="background-image: url(\''.$auction['auction_image'].'\');" alt="'.$title .'"/></a>'
						.'<a class="title_allegro" target="_blank" href="'.$url.'" title="'.$title.'">'.$title.'</a>'
                        ...
                        .((isset($attrs['show_details']) && $attrs['show_details'] == 1) ? '<a href="'.$url.'" class="title_allegro '.($item_allegro_model->getItemById($id) ? "show_auction_details" : "").'" title="'.$title.'" target="_blank">'.__('Details','gj_myauctions_allegro').'</a>' : '')
					.'</p>'
				.'</div>';
			}
            if(isset($attrs['show_comments']) && $attrs['show_comments']){
                $feedbackModel = new GjmaaFeedbackItem();
                $html .= $feedbackModel->addCommentSection($api_allegro,$user_id,$attrs['count_of_comments']);
            }
		} else {
